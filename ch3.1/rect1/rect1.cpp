...
struct Point{
    long x;
    long y;
    Point(long inX, long inY): x(inX), y(inY) {};
    Point(): x(0), y(0) {};
};
...
int g_nHeight = 0;
int g_nRectangles = 0;
long g_ColorAreas[MAX_COLORS];
std::list<rectangle> g_listRectangles;
long GetArea(const rectangle &r) {
    return (r.PtUr.x-r.PtLl.x+1)*(r.PtUr.y-r.PtLl.y+1);
}

void PrintRect(const rectangle &r) {
    dprintf(("(%ld %ld), (%ld %ld), area: %ld, color: %d", r.PtLl.x, r.PtLl.y, r.PtUr.x, r.PtUr.y, GetArea(r), r.color));
}

void PrintPoint(const Point &pt) {
    dprintf(("(%ld %ld)", pt.x, pt.y));
}
...
bool DoesLineIntersectRectangle(line myLine, rectangle rectIn) {
    ...
    return DoesAxisLineIntersectLine(myLine, line(Point(rectIn.PtLl.x, rectIn.PtLl.y),
                                                  Point(rectIn.PtUr.x, rectIn.PtUr.y)));
};
...
int main(){
    ...
    rectIn.PtLl.x = 0;
    rectIn.PtLl.y = 0;
    rectIn.PtUr.x = g_nWidth-1;
    rectIn.PtUr.y = g_nHeight-1;
    rectIn.color = 1;
    g_listRectangles.push_front(rectIn);
    dprintf(("Inserted rectangle (%ld, %ld), (%ld, %ld), %d into our list\n", 
    ...
    for (int i=0; i < g_nRectangles; i++){
        ...
        fscanf(fin, "%ld %ld %ld %ld %d\n",
               &rectIn.PtLl.x, &rectIn.PtLl.y, &rectIn.PtUr.x, &rectIn.PtUr.y, &rectIn.color);
        rectIn.PtUr.x--;
        rectIn.PtUr.y--;
        dprintf(("processing ")); PrintRect(rectIn); dprintf(("\n"));
    ...
    for (std::list<rectangle>::iterator iter = g_listRectangles.begin();
         ...
         iter++) {
        nArea = GetArea(*iter);
        PrintRect(*iter); dprintf((" nArea: %ld\n", nArea));
    ...
    for (int i = 0; i < MAX_COLORS; i++){
        ...
        if (g_ColorAreas[i] >0){
            fprintf(fout, "%d %ld\n", i, g_ColorAreas[i]);
            nColorSum += g_ColorAreas[i];
        }
    }
    dprintf(("Sum of Colors: %ld\n", nColorSum)); 
    exit(0);
